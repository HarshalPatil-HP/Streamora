...
const getAllVideos = asynchandler(async (req, res) => {
  ...
  if (sort === "latest") {
    ...
    resolvedSortType = "desc";
  }
  if (sort === "trending") {
    resolvedSortBy = "views";
    resolvedSortType = "desc";
  }
...
const publishAVideo = asynchandler(async (req, res) => {
  ...
  if (!videoLocalPath) {
    throw new apireject(
      400,
      "Video file is required. Please select a video to upload.",
    );
  }
  ...
  if (!thumbnailLocalPath) {
    throw new apireject(
      400,
      "Thumbnail is required. Please select a thumbnail image.",
    );
  }
  ...
  if (!videoFile) {
    throw new apireject(
      500,
      "Video upload failed. Please check your file format (MP4, WebM) and try again.",
    );
  }
  ...
  if (!thumbnail) {
    throw new apireject(
      500,
      "Thumbnail upload failed. Please check your image format (JPG, PNG, WebP) and try again.",
    );
  }
