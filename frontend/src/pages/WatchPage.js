...
import { toggleVideoLike, getVideoLikeStatus, toggleCommentLike } from "../services/likeService.js";
import { toggleSubscribe } from "../services/subscriptionService.js";
import { getChannelProfile } from "../services/authService.js";
import { getUserPlaylists, createPlaylist, addVideoToPlaylist, removeVideoFromPlaylist } from "../services/playlistService.js";
...
export async function mountWatchPage(params) {
  ...
  try {
    ...
    const [video, commentsResult, recommended, likeStatus] = await Promise.all([
      ...
      isAuthenticated ? getVideoLikeStatus(id).catch(() => ({ liked: false, likeCount: 0 })) : Promise.resolve({ liked: false, likeCount: 0 }),
    ]);
    ...
    const isOwner = isAuthenticated && String(user?._id) === String(video.owner?._id || video.owner);
        ...
        <div class="xl:col-span-2 space-y-4">
          ...
          <div class="surface-card" id="comments-section">
            ...
            <div id="comments-list" class="space-y-5">
              ${comments.length
                ? comments.map((c) => renderComment(c, isAuthenticated)).join("")
    ...
    likeBtn?.addEventListener("click", async () => {
      ...
      likeBtn.className = newLiked ? "btn-liked gap-2 text-xs" : "btn-secondary gap-2 text-xs";
      ...
      try {
        ...
        likeBtn.className = serverLiked ? "btn-liked gap-2 text-xs" : "btn-secondary gap-2 text-xs";
        if (icon)  icon.setAttribute("fill", serverLiked ? "currentColor" : "none");
        if (label) label.textContent = serverLiked ? "Liked" : "Like";
        if (count) { count.textContent = serverCount > 0 ? formatViews(serverCount) : ""; count.className = serverLiked ? "text-red-400" : "text-[#ABABAB]"; }
        showToast(serverLiked ? "Liked!" : "Unliked", "success");
      } catch (err) {
        // Rollback
        likeBtn.dataset.liked = String(wasLiked);
        likeBtn.className = wasLiked ? "btn-liked gap-2 text-xs" : "btn-secondary gap-2 text-xs";
    ...
    document.getElementById("subscribe-btn")?.addEventListener("click", async () => {
      ...
      try {
        ...
        if (btn) {
          ...
          btn.className = result?.subscribed ? "btn-secondary text-xs shrink-0" : "btn-primary text-xs shrink-0";
        }
    ...
    document.getElementById("comments-list")?.addEventListener("click", async (e) => {
      ...
      try {
        ...
        if (count) count.textContent = result.likeCount > 0 ? result.likeCount : "";
      } catch (err) {
        // Rollback
    ...
    document.getElementById("show-new-playlist-form-btn")?.addEventListener("click", (e) => {
    ...
    if (playlistForm) {
      playlistForm.onsubmit = async (e) => {
        ...
        try {
          ...
          document.getElementById("show-new-playlist-form-btn")?.classList.remove("hidden");
...
async function loadPlaylistModal(videoId) {
  ...
  try {
    ...
    listEl.innerHTML = list.map((pl) => {
      const hasVideo = pl.videos?.some((v) => String(v) === String(videoId) || String(v?._id) === String(videoId));
      return `
